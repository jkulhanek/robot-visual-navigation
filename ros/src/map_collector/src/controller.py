...
class Controller:
    ...
    def auto_collect(self):
        ...
        while not rospy.is_shutdown():
            ...
            rot = self._rotation[2]

            collect = False
            if lastPosition is None or lastRotation is None:
                collect = True
            elif np.linalg.norm(pos - lastPosition) >= 0.05 or \
                abs(rot - lastRotation) >= pi / 24:
                collect = True

            if collect:
                self.collect(self.observe(), (float("NaN"), float("NaN")), float("NaN"))
                lastPosition = pos
                lastRotation = rot
            if self.visualize is not None and rospy.Time.now() - time > rospy.Duration(1):
                self.visualize()
                time = rospy.Time.now()
            rate.sleep()
    ...
    def move_by(self, distance):
        ...
        while not rospy.is_shutdown():
            ...
            msg = Twist()            
            speed = M_SPEED if abs(dir_distance) > 0.2 else (M_SPEED / 3)
    ...
    def rotate_to(self, angle):
        ...
        while not rospy.is_shutdown():
            ...
            msg = Twist()            
            speed = R_SPEED if abs(diff) > 0.3 else (R_SPEED / 3)
